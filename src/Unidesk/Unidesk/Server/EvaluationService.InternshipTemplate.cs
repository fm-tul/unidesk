...
{
    private Document GetInternshipPdf(EvaluationDetailDto item, string? permalink = null)
    {
        var bytes = (byte[]?)null;
        if (permalink.IsNotNullOrEmpty())
        {
            var qr = QrCode.EncodeText($"{_serverService.UrlBase}/{permalink}", QrCode.Ecc.Low);
            var skBitmap = qr.ToBitmap(10, 1, SKColors.Gray, SKColors.White);
            var memoryStream = new MemoryStream();
            SKData data = SKImage.FromBitmap(skBitmap).Encode(SKEncodedImageFormat.Png, 100);
                                   ...
                                   .Column(c =>
                                    {
                                        ...
                                        var simpleAnswers = new[]
                                            { Questions.TextQuestions.InternshipStudentName_CZ, Questions.TextQuestions.InternshipStudyProgram_CZ, Questions.TextQuestions.InternshipStudentId_CZ };
                                        ...
                                        c.Item()
                                           ...
                                           .PaddingTop(25)
                                           .Text(
                                                $"Potvrzuji, že výše uvedený student absolvoval praxi ve společnosti {companyName} od {dateFrom} do {dateTo} na pracovní pozici {position} v rozsahu 180 hodin.")
                                           .FontFamily("Calibri");
